...
import {
  useListPickupPointApplications,
  useApprovePickupPointApplication,
  useRejectPickupPointApplication,
...
function PickupPointRow({
  ...
  app: PickupPointApplication;
  showActions: boolean;
  onApprove: (app: PickupPointApplication) => void;
  onReject: (app: PickupPointApplication) => void;
}) {
...
export function AdminPickupPointsPage() {
  const { data: applications = [], isLoading } = useListPickupPointApplications();
  const approve = useApprovePickupPointApplication();
  const reject = useRejectPickupPointApplication();
  ...
  const handleApprove = async (app: PickupPointApplication) => {
    try {
      await approve.mutateAsync(app.applicant);
      toast.success(`${app.name} approved as pickup point`);
    } catch {
      toast.error('Failed to approve application');
    }
  };

  const handleReject = async (app: PickupPointApplication) => {
    try {
  ...
  const renderTable = (apps: PickupPointApplication[], showActions: boolean) => {
    if (isLoading) {
      return (
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Review and manage pickup point applications</p>
      </div>
